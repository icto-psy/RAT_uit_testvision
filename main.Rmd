...
# Script voor het berekenen van RAT waardes.

## Stappenplan voor gebruik:

### Download resultaten uit Testvision

1. Ga via **Gepublieerde toetsen** naar het tentamen. ![](www/pictures/pubtests.PNG)
...
#### HIER HET PAD NAAR DE INPUT FILE INVOEGEN ####

data <- 
  read.table(file = "input/testinputfile.csv",header = FALSE,as.is = TRUE,fill = TRUE,sep = ",", comment.char = "")
...
names_and_answers <-
  data[seq(which(data$V2 == "QId") + 1,dim(data)[1],by = 2),2:dim(data)[2]]
...
na_columns <- which(apply(answers,2,function(x) sum(is.na(x)) == dim(answers)[1]))

mc_columns <- which(apply(answers,2,function(x) any(grepl("[A-Z]{1}",x))))

if(length(mc_columns) == 0) stop("NO MC QUESTIONS")

other_columns <- which(apply(answers,2, function(x) sum(grepl("-2|^$",x)) == dim(answers)[1]))

if(any(other_columns == 1)){
 other_columns <- other_columns[-which(other_columns == 1)]
}

other_columns <- other_columns[!((other_columns - 1) %in% mc_columns)]
...
#mc_answers_factorized <- 
  max_answer_alternative_filtered <- max_answer_alternative[mc_columns]
  
  possible_mc_answer_factor_levels <- list()
  for(i in seq_along(max_answer_alternative_filtered)) {
   possible_mc_answer_factor_levels[[i]] <- LETTERS[1:which(LETTERS == max_answer_alternative_filtered[i])]
  } 
  
 mc_answers_factorized <- mc_answers
 for(i in seq_along(mc_answers[1,])) {
   mc_answers_factorized[ , i] <- factor(mc_answers[ , i],levels = possible_mc_answer_factor_levels[[i]] )
 }
...
# Calculating RAT score for each answer category.
 for(i in seq_along(mc_answers[ 1, ])){
   for(j in seq_along(possible_mc_answer_factor_levels[[i]])){
     rat_values[[i]][j] <- round(cor(mc_answers[ , i] == possible_mc_answer_factor_levels[[i]][j], total_score),3)
   }
...
#### HIER HET PAD NAAR DE GEWENSTE OUTPUT FILE TOEVOEGEN ####
...
Let op: De vraag met de meeste antwoord opties bepaald hoe veel antwoordopties in de linkerkolom komen te staan. 
